...
const TeamData = () => {
  ...
  useEffect(() => {
    ...
    if (teamValue) {
      axios.get(`http://localhost:8081/api/v1/player?team=${encodeURIComponent(teamValue)}`)
        .then(response => {
      ...
      axios.get(`http://localhost:8081/api/v1/player?nation=${encodeURIComponent(nationValue)}`)
        .then(response => {
          setPlayerData(response.data);
          setLoading(false);
        })
      ...
      axios.get(`http://localhost:8081/api/v1/player?position=${encodeURIComponent(positionValue)}`)
        .then(response => {
          setPlayerData(response.data);
          setLoading(false);
        })
      ...
      axios.get(`http://localhost:8081/api/v1/player?name=${encodeURIComponent(nameValue)}`)
        .then(response => {
          setPlayerData(response.data);
          setLoading(false);
        })
